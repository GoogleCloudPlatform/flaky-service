...
describe('flaky express server', () => {
  ...
  it('should send back an error code when being sent invalid metadata', async () => {
    ...
    const resp = await fetch('http://127.0.0.1:3000/postbuild', {
      method: 'post',
  ...
  after(async () => {
    ...
    var parsedPayload = JSON.parse(EXAMPLE_PAYLOAD);
    var repoId = encodeURIComponent(parsedPayload.metadata.github.repository);
    const buildId = parsedPayload.metadata.github.sha;
    ...
    await client.collection(global.headCollection).doc(repoId).delete();
  });
});
